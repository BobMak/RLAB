...
class PPO(PolicyGradients):
    ...
    def backward(self):
        ...
        if self.use_wandb:
            ...
            wandb.log({"avgAdvantage": adv.mean()})
        ...
        for _ in range(80):
            ...
            # clip it
            surr1 = torch.clamp(ratio, min=1 - self.clip_ratio, max=1 + self.clip_ratio) * adv.unsqueeze(1)
            surr2 = ratio * adv.unsqueeze(1)
        ...
        pred_values = self.getExpectedvalues(self.train_states)
        critic_loss = torch.nn.MSELoss()(pred_values.flatten(), r)
        print("\ntrain reward", self.train_rewards.mean(), "advtg", adv.mean(), "critic loss", critic_loss)
